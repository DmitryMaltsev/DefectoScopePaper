...
namespace DefectoScope.OffCollection
{
    ...
    [Serializable]
    public class OffList<T> : IList<T>, IList, IReadOnlyList<T>
    {
        #region Операторы
        ...
        /// <summary>
        ///     Создает список, который изначально пуст и имеет нулевую емкость.
        ///     После добавления первого элемента в список емкость увеличивается до 16,
        ///     а затем увеличивается по мере необходимости на два.
        /// </summary>
        public OffList() => _items = EmptyArray;


        /// <summary>
        ///     Создает список с заданной начальной емкостью. Список изначально пуст,
        ///     но в нем будет место для заданного количества элементов,
        ///     прежде чем потребуется какое-либо перераспределение.
        /// </summary>
        /// <param name="capacity"></param>
        public OffList(int capacity)
        {
            if (capacity < 0)
            {
        ...
        /// <summary>
        ///     Создает список, копируя содержимое данной коллекции.
        ///     Размер и емкость нового списка будут равны размеру данной коллекции.
        /// </summary>
        /// <param name="collection"></param>
        public OffList(IEnumerable<T> collection)
        {
            if (collection == null)
            ...
            else
            {
                ...
                //Этот перечислимый может быть пустым. Пусть Add выделяет новый массив, если это необходимо.
                //Обратите внимание, что сначала также будет идти _defaultCapacity, а не 1, затем 2 и т. д.
                if (collection != null)
                {
        ...
        /// <summary>
        ///     Получает и устанавливает емкость этого списка. Емкость - это размер внутреннего массива,
        ///     используемого для хранения элементов. Когда установлено,
        ///     внутренний массив списка перераспределяется на заданную емкость.
        /// </summary>
        public int Capacity
        {
            get => _items.Length;
            set
            {
                if (value < Count)
                {
                    OffThrowHelper.ThrowArgumentOutOfRangeException(ExceptionArgument.Value,
        ...
        /// <summary>
        ///     Гарантирует, что емкость этого списка по крайней мере соответствует заданному минимальному значению.
        ///     Если текущая емкость списка меньше, чем min, емкость увеличивается вдвое по сравнению с текущей емкостью или до
        ///     min,
        ...
        private void EnsureCapacity(int min)
        {
            if (_items.Length < min)
            {
                ...
                //Позвольте списку расти до максимально возможной емкости (~ 2G элементов), прежде чем возникнет переполнение.
                //Обратите внимание, что эта проверка работает, даже если _items.Length переполнен благодаря приведению (uint)
        ...
        public OffList<T> FindAll(Predicate<T> match)
        {
            if (match == null) OffThrowHelper.ThrowArgumentNullException(ExceptionArgument.Match);
            else
            {
                var list = new OffList<T>();
                for (var i = 0; i < Count; i++)
                {
                    if (match(_items[i]))
                        list.Add(_items[i]);
        ...
        public OffList<T> GetRange(int index, int count)
        {
            if (index < 0)
            {
                OffThrowHelper.ThrowArgumentOutOfRangeException(ExceptionArgument.Index,
        ...
        /// <summary>
        ///     Устанавливает емкость этого списка в размер списка.
        ///     Этот метод можно использовать для минимизации накладных расходов на память списка,
        ///     если известно, что новые элементы не будут добавлены в список. Чтобы полностью очистить список
        ///     и освободить всю память, на которую ссылается этот список, выполните следующие инструкции:
        ...
        internal static IList<T> Synchronized(OffList<T> list) => new SynchronizedList(list);
        ...
        {
            private readonly OffList<T> _list;
            private readonly object _root;

            internal SynchronizedList(OffList<T> list)
            {
                _list = list;
        ...
        public struct Enumerator : IEnumerator<T>
        {
            ...
            internal Enumerator(OffList<T> list)
            {
                _list = list;
                _index = 0;
